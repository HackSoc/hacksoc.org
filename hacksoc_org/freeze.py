...
def get_static_routes() -> Generator[str, None, None]:
    """Yields URL routes for every file in static/, whether it's used in the 
    site or not. The site templates (currently) don't use url_for for static
    assets, and static assets may be used for off-site purposes, so it's better
    to be explicit in this case.
...
def get_server_page_routes() -> Generator[str, None, None]:
    """Yields URL routes for the server pages, since they're not linked from the 
    main website.
...
def freeze():
    """ this seemingly useless method is used to provide a stable API if the
    freeze provider changes in the future

    https://pythonhosted.org/Frozen-Flask/#finding-urls
    Currently flask_frozen will implicitly find routes (in our case) if there is
    a url_for call to them. Since none of the static assets are loaded like
    this, and the server pages are not linked to at all from the main site, the
    two above generators provide routes for them. 
    
    The unused get_all_routes() function can be adapted in the future if 
    explicit route listing is desired.
    """
