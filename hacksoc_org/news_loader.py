...
class MarkdownNewsLoader(BaseLoader):
    ...
    def __init__(self, searchpath : str, prefix_allow : Optional[str]=None) -> None:
        ...
        Args:
            ...
            prefix_allow ([str], optional): if provided, only allow `template`s matching the provided prefix
        """
        self.prefix_allowlist = prefix_allow
        self.searchpath = searchpath
        super().__init__()

    def get_source(self, environment: Environment, template: str) -> Tuple[str, str, Optional[Callable[[],bool]]]:
        """Superclass override (https://jinja.palletsprojects.com/en/3.0.x/api/#loaders)

        When told to find a template with name `foo.html.jinja2`, will attempt
        to find a template with name `foo.md` and wrangle it into Jinja format.
